...
export class OpenaiBrain extends Brain implements IBrain {
    ...
    private initializeLLM = (): OpenAI => {
        ...
        const model = process.env.BOTLANDIA_BACKEND_OPENAI_GPT_MODEL_NAME;
        if (!apiKey || !model) {
            const errorMessage = "Variáveis de ambiente BOTLANDIA_BACKEND_OPENAI_KEY ou BOTLANDIA_BACKEND_OPENAI_GPT_MODEL_NAME não estão definidas.";
    ...
    thinkAbout = async (about: string, tools: any[]): Promise<IBill> => {
        ...
        try {
            ...
            const getBody = () => {
                ...
                if (funcs) {
                    return {
                        model: process.env.BOTLANDIA_BACKEND_OPENAI_GPT_MODEL_NAME!,
